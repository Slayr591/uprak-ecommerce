    ...
    <div class="app-canvas !block !min-h-[calc(100vh-52px)]">
        <header class="h-16 bg-[#fcfcfd] border-b border-[#e3e6eb] px-7 flex items-center justify-between">
            ...
            <nav class="flex items-center gap-8 text-sm font-semibold text-[#111827]">
                <a href="{{ route('user.products') }}">Beranda</a>
                <a href="{{ route('user.products') }}">Produk</a>
                <a href="{{ route('user.products') }}">Bantuan</a>
                <a href="{{ route('login', ['redirect' => $redirectTarget]) }}" class="btn-dark !h-10 !px-5">Masuk</a>
            </nav>
        </header>
            ...
            <div class="w-full max-w-[520px] ui-card rounded-2xl p-8 border-t-4 border-t-[#74f0c2]">
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required class="ui-input" placeholder="Masukkan nama lengkap">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-[#111827] mb-1.5">Email</label>
                        <input type="email" name="email" value="{{ old('email') }}" required class="ui-input" placeholder="nama@email.com">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" value="{{ old('phone') }}" class="ui-input" placeholder="+62 812 3456 7890">
                    </div>
                ...
                <div class="mt-6 border-t border-[#eceff3] pt-6 text-center text-sm text-[#6b7280]">
                    Sudah memiliki akun? <a href="{{ route('login', ['redirect' => $redirectTarget]) }}" class="font-bold text-[#111827]">Masuk di sini</a>
                </div>
            </div>
        </main>
