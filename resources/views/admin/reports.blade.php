    ...
    <header class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
      ...
      <div class="flex items-center gap-4">
        <form method="GET" class="relative">
          ...
          <input type="text" name="search" value="{{ $search ?? request('search') }}" placeholder="Cari produk, pelanggan, pembayaran..." class="pl-10 pr-4 py-2 bg-gray-100 rounded-lg text-sm w-64 focus:outline-none">
        </form>
    ...
    <main class="flex-1 overflow-y-auto p-8">
        ...
        <div class="flex items-start justify-between">
          ...
          <form method="GET" class="flex items-center gap-3">
            ...
            <label class="flex items-center gap-2 bg-white border border-gray-200 px-4 py-2 rounded-lg text-sm font-medium">
              ...
              <input type="month" name="month" value="{{ $selectedMonth->format('Y-m') }}" onchange="this.form.submit()" class="bg-transparent text-sm focus:outline-none">
            </label>
          </form>
        </div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <p class="text-2xl font-bold text-gray-900 mb-1">{{ $stats['low_stock'] }}</p>
          <p class="text-xs text-red-500 flex items-center gap-1">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
            {{ $stats['low_stock'] > 0 ? $stats['low_stock'] . ' produk perlu restock' : 'Stok aman' }}
          </p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6">
          <p class="text-xs font-medium text-gray-500 uppercase mb-1">New Customers</p>
          <p class="text-2xl font-bold text-gray-900 mb-1">{{ $stats['new_users_month'] }}</p>
          <p class="text-xs text-green-500 flex items-center gap-1">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>
            {{ $stats['new_users_growth'] >= 0 ? '+' : '' }}{{ $stats['new_users_growth'] }}% vs last month
          </p>
        </div>
        <div class="bg-green-100 rounded-xl border border-green-200 p-6">
          ...
          <p class="text-2xl font-bold text-gray-900 mb-1">{{ number_format($stats['conversion_rate'], 2) }}%</p>
          <p class="text-xs text-green-600 flex items-center gap-1">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            {{ $stats['paid_orders_month'] }} paid of {{ $stats['orders_month'] }} orders
          </p>
        </div>
      </div>
            ...
            <tr class="hover:bg-gray-50">
              <td class="px-6 py-3">
                <div class="text-sm font-medium text-gray-900">
                  ...
                  <p>{{ $item->product->name ?? $item->product_name }}</p>
                  @endforeach
