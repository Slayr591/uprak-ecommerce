  ...
  <div class="flex-1 flex flex-col overflow-hidden bg-[#f7f8fa]">
    <header class="h-16 bg-[#fbfbfc] border-b border-[#dfe3e8] px-6 py-3 flex items-center justify-between">
      ...
      <nav class="flex items-center gap-8 text-sm font-medium text-[#717785]">
        <a href="{{ route('admin.users.index') }}" class="text-black font-bold border-b-2 border-black pb-1">Users</a>
        <a href="{{ route('admin.products.index') }}">Inventory</a>
        <a href="{{ route('admin.orders.index') }}">Orders</a>
        <a href="{{ route('admin.permissions') }}">Settings</a>
    ...
    <main class="flex-1 overflow-y-auto p-8">
      ...
      <div class="max-w-5xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          ...
          <a href="{{ route('admin.users.create') }}" class="btn-dark !h-11 !px-6">
            ...
            <tr class="hover:bg-gray-50">
              <td class="px-6 py-4 bg-white">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 bg-[#e9ebef] rounded-full flex items-center justify-center text-[#111827] text-sm font-bold">{{ substr($user->name,0,2) }}</div>
                  <div>
