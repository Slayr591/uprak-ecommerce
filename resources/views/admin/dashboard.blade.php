    ...
    <main class="flex-1 overflow-y-auto p-8">
      ...
      <div class="grid grid-cols-3 gap-6 mb-8">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <p class="text-xs {{ $stats['user_growth'] >= 0 ? 'text-green-500' : 'text-red-500' }}">{{ $stats['user_growth'] >= 0 ? '+' : '' }}{{ $stats['user_growth'] }}% vs last month</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6">
          <div class="flex items-center justify-between mb-2">
          ...
          <p class="text-3xl font-bold text-gray-900 mb-1">{{ $stats['total_staff'] }}</p>
          <p class="text-xs {{ $stats['staff_growth'] >= 0 ? 'text-green-500' : 'text-red-500' }}">{{ $stats['active_staff'] }} active · {{ $stats['staff_growth'] >= 0 ? '+' : '' }}{{ $stats['staff_growth'] }}% vs last month</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6">
          <div class="flex items-center justify-between mb-2">
          ...
          <p class="text-xs {{ $stats['revenue_growth'] >= 0 ? 'text-green-500' : 'text-red-500' }}">{{ $stats['revenue_growth'] >= 0 ? '+' : '' }}{{ $stats['revenue_growth'] }}% growth vs last month</p>
        </div>
      </div>
      <div class="grid grid-cols-3 gap-6 mb-8">
        <div class="col-span-2 bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mt-5">
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
              <p class="text-[11px] font-semibold text-gray-500 uppercase">{{ $label }}</p>
              <p class="mt-1 text-xs font-bold text-gray-900">{{ \App\Helpers\CurrencyHelper::rupiah($chartData[$index] ?? 0) }}</p>
            </div>
            @endforeach
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <div class="flex items-center justify-between">
                  ...
                  <span class="px-2 py-0.5 {{ $activity['type'] == 'admin' ? 'bg-black text-white' : ($activity['type'] == 'staff' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700') }} text-xs rounded-full">{{ strtoupper($activity['type']) }}</span>
                </div>
          ...
          <a href="{{ route('admin.reports') }}" class="block w-full mt-4 py-2 bg-gray-100 rounded-lg text-sm font-medium text-gray-700 text-center hover:bg-gray-200 transition-colors">View All Activity</a>
        </div>
      </div>
