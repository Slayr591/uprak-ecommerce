      ...
      <div class="w-80 flex-shrink-0">
        <div class="rounded-2xl bg-[#0d1938] text-white p-6 shadow-xl">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex justify-between"><span class="text-[#c4cada]">Order ID</span><span class="font-medium">{{ $order->order_number }}</span></div>
            <div class="flex justify-between"><span class="text-[#c4cada]">Subtotal</span><span>{{ \App\Helpers\CurrencyHelper::rupiah($order->subtotal) }}</span></div>
            <div class="flex justify-between"><span class="text-[#c4cada]">Ongkir</span><span>{{ \App\Helpers\CurrencyHelper::rupiah($order->shipping_cost) }}</span></div>
            <div class="flex justify-between"><span class="text-[#c4cada]">Pajak</span><span>{{ \App\Helpers\CurrencyHelper::rupiah($order->tax) }}</span></div>
            <div class="border-t border-[#2a3b63] pt-2 flex justify-between font-bold"><span>Total Bayar</span><span class="text-2xl text-[#3df79f]">{{ $order->total_formatted }}</span></div>
          </div>
        </div>
      </div>
    </div>
