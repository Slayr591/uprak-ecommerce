@extends('layouts.app')
@section('title','Profil Saya - UKK E-Commerce')
@section('content')
  ...
  <header class="bg-white border-b border-gray-200">
    <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
      ...
      <nav class="flex items-center gap-6 text-sm">
        <a href="{{ route('user.products') }}" class="text-gray-600 hover:text-gray-900">Home</a>
        <a href="{{ route('user.history') }}" class="text-gray-600 hover:text-gray-900">History</a>
        <a href="{{ route('user.profile') }}" class="text-black font-medium border-b-2 border-black pb-1">Account</a>
      </nav>
    </div>
  </header>
      ...
      <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <div class="flex flex-col items-center text-center">
          @if($user->profile_photo_url)
            <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}" class="w-24 h-24 rounded-full object-cover border border-gray-200 shadow-sm">
          @else
            <div class="w-24 h-24 rounded-full bg-black text-white flex items-center justify-center text-3xl font-bold shadow-sm">
              {{ strtoupper(substr($user->name, 0, 1)) }}
            </div>
          @endif
          <h1 class="mt-4 text-xl font-bold text-gray-900">{{ $user->name }}</h1>
          ...
          <div class="mt-4 w-full space-y-3 text-left text-sm">
            ...
            <div class="rounded-xl bg-gray-50 border border-gray-200 px-4 py-3">
              ...
              <p class="font-medium text-gray-900">{{ $user->address ?: 'Belum diisi' }}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <form method="POST" action="{{ route('user.profile.update') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" name="name" value="{{ old('name', $user->name) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Email</label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black">
              </div>
            </div>
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Nomor Telepon</label>
                <input type="text" name="phone" value="{{ old('phone', $user->phone) }}" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black">
              </div>
              <div>
            ...
            <div>
              ...
              <textarea name="address" rows="4" class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-black">{{ old('address', $user->address) }}</textarea>
            </div>
